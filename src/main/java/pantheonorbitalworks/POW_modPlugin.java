...
import pantheonorbitalworks.plugins.*;

public class POW_modPlugin extends BaseModPlugin {
		
    @Override
    public void onNewGame() {
        //Global.getSector().registerPlugin(new RefitCampaignPlugin());
    ...
    public void onNewGameAfterEconomyLoad() {
        ...
        if (market != null) {
			...
			for (PersonAPI person : people) {
				if (person.getName().getLast() != "Torgue") {
                    ...
                    mrTorgue.getName().setFirst("Mister");
                    mrTorgue.getName().setLast("Torgue");
